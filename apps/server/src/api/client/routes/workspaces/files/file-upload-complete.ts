...
interface FileUploadCompleteParams {
  workspaceId: string;
  fileId: string;
}

interface FileUploadCompleteBody {
  uploadId: string;
}

export const fileUploadCompleteRoute: FastifyPluginCallback = (
  instance,
  _,
  done
) => {
  instance.put<{
    ...
    Body: FileUploadCompleteBody;
  }>('/:fileId', async (request, reply) => {
